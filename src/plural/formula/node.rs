...
pub(crate) enum Node {
    Var,
    Num(i64),
    UnOp { op: UnOp, rhs: Box<Node> },
    BinOp { op: BinOp, lhs: Box<Node>, rhs: Box<Node> },
    Cond { test: Box<Node>, if_true: Box<Node>, if_false: Box<Node> },
}
...
impl Node {
    ...
    pub(super) fn new_binop(op: BinOp, lhs: Node, rhs: Node) -> Node {
        Node::BinOp { op, lhs: Box::new(lhs), rhs: Box::new(rhs) }
    }

    pub(super) fn new_cond(test: Node, if_true: Node, if_false: Node) -> Node {
        Node::Cond { test: Box::new(test), if_true: Box::new(if_true), if_false: Box::new(if_false) }
    }
    ...
    pub(super) fn execute(&self, n: i64) -> i64 {
        match self {
            ...
            Node::BinOp { op, lhs, rhs } => {
                ...
                match op {
                    ...
                    BinOp::Div => {
                        ...
                        if rhs != 0 { lhs.overflowing_div(rhs).0 } else { get_infinity(lhs) }
                    }
                    BinOp::Mod => {
                        let rhs = rhs.execute(n);

                        if rhs != 0 { lhs.overflowing_rem(rhs).0 } else { lhs }
                    }
            ...
            Node::Cond { test, if_true, if_false } => if test.execute(n) != 0 {
                if_true.execute(n)
            } else {
                if_false.execute(n)
            }
...
impl PartialEq<Self> for Node {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Node::BinOp { op: lo, lhs: ll, rhs: lr }, Node::BinOp { op: ro, lhs: rl, rhs: rr }) => {
                (lo == ro) && (ll == rl) && (lr == rr)
            }
            (
                Node::Cond { test: lc, if_true: lt, if_false: lf },
                Node::Cond { test: rc, if_true: rt, if_false: rf },
            ) => {
                (lc == rc) && (lt == rt) && (lf == rf)
            }
            _ => false,
...
mod tests {
    ...
    fn test_enum_node() {
        ...
        n = Node::UnOp { op: UnOp::Not, rhs: Box::new(Node::Var) };
        assert_ne!(n.clone(), Node::Var);
        assert_eq!(format!("{:?}", n), String::from("UnOp { op: Not, rhs: Var }"));

        n = Node::BinOp { op: BinOp::Add, lhs: Box::new(Node::Var), rhs: Box::new(Node::Var) };
        assert_ne!(n.clone(), Node::Var);
        assert_eq!(format!("{:?}", n), String::from("BinOp { op: Add, lhs: Var, rhs: Var }"));

        n = Node::Cond { test: Box::new(Node::Var), if_true: Box::new(Node::Var), if_false: Box::new(Node::Var) };
        assert_ne!(n.clone(), Node::Var);
        assert_eq!(format!("{:?}", n), String::from("Cond { test: Var, if_true: Var, if_false: Var }"));
    }
